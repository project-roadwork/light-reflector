...
| `register.addSource` | Registers a source | source: `BasePart`, isEnabled: `ValueBase` or `boolean` or `number`, getIntensity: `(source: BasePart) -> number`, getColor: `(source: BasePart) -> (vector)` | `()` |
| `register.removeSource` | Unregisters a source | source: `BasePart` | `()` |
...
::alert{type="warning" icon="lucide:pencil"}
