 ...
 * --[[
     ...
     SPDX-FileCopyrightText: 2024 jecs authors
...
type IterFn<T extends Id[]> = IterableFunction<
...
export class World {
  ...
  add<C>(
    entity: Entity,
    component: TagDiscriminator extends InferComponent<C>
      ? C
      : Id<TagDiscriminator>,
  ): void;
  ...
  set<T>(
    component: Entity<T>,
    hook: HookWithDeleted,
    value: (e: Entity, id: Id<T>, deleted?: true) => void,
  ): void;
  /**
   ...
   */
  set<E extends Id<unknown>>(
    entity: Entity,
    component: E,
    value: InferComponent<E>,
  ): void;
  ...
  removed<T>(
    component: Entity<T>,
    listener: (e: Entity, id: Id<T>, deleted?: true) => void,
  ): () => void;
}
...
type HookWithDeleted = Entity<
  <T>(e: Entity<T>, id: Id<T>, deleted?: true) => void
> & {
...
export function bulk_insert<const C extends Id[]>(
  ...
  ids: C,
  values: TrimOptional<{
    [K in keyof C]: TagToUndefined<InferComponent<C[K]>>;
  }>,
): void;
...
export function record<T extends Id[] = []>(
  world: World,
  entity: Entity,
): EntityRecord<T>;
